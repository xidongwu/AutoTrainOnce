...
class AdamW(Optimizer):
    ...
    def get_terms(self):
        ...
        return self.bc_1, self.bc_2, self.exp_avg, self.exp_avg_sq
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                state['step'] += 1
                #if p.data.size() in self.size_list and self.size_list is not None:
                ...
                #if p.data.size() in self.size_list and self.size_list is not None:
                self.bc_1.append(bias_correction1)
                self.bc_2.append(bias_correction2)
                # self.exp_avg.append(exp_avg)
                # self.exp_avg_sq.append(exp_avg_sq)
                ...
                #biased_p = p.data.clone()

                #biased_p[biased_p>=0.5] = -(2*(p.data[biased_p>=0.5]-0.5)-1).pow(2)
                #biased_p[biased_p >= 0.5] = -1*biased_p[biased_p >= 0.5].uniform_(0,1)

                #biased_p.mul_(p.data)
                #p.data.add_(-group["weight_decay"], biased_p).addcdiv_(-step_size, exp_avg, denom)

                #p.data.add_(-group["weight_decay"], p.data).addcdiv_(-step_size, exp_avg, denom)
        # print(len(self.exp_avg))
        # print(self.exp_avg[0].size())
        # print(self.exp_avg[1].size())
        # print(self.exp_avg[2].size())
        # print(self.exp_avg[3].size())
        # print(self.exp_avg[-1].size())
        return loss
...
class DHSPG(Optimizer):
    ...
    def __init__(self, params, variant='sgd', lr=required, lmbda=None, lmbda_amplify=None, hat_lmbda_coeff=None, epsilon=0.0, first_momentum=None, second_momentum=None, dampening=None, weight_decay=None, 
        ...
        if partition_step is None or half_space_project_steps is None:
            ...
            self.partition_step = start_pruning_steps  # 15
    ...
    def get_xs_grads(self, group, require_grad=True):
        xs = []
        grads = []
        # params || auxiliary_ccs
        for i, param in enumerate(group['params']):
            if param.grad is None and require_grad:
                continue
            if len(param.data.shape) == 1:
                xs.append(param.data.view(group['num_groups'], -1)) # x
                if require_grad:
    ...
    def partition_groups(self):
        ...
        for i, group in enumerate(self.param_groups):
            if group['group_type'] >= GROUP_TYPE['multi-head-linear']:
                # linear | conv
                xs, grads = self.get_xs_grads(group)  # get xs and grads
                flatten_x = torch.cat(xs, dim = 1) # channel * -1
                flatten_grad = torch.cat(grads, dim = 1)
                group_size = flatten_x.shape[1] if len(flatten_x.shape) > 1 else 1
                cos_sim = F.cosine_similarity(-flatten_x, -flatten_grad) # cos x and \nabla f
                ...
                total_num_groups += group['num_groups'] # cnt group number
        ...
        for i, group in enumerate(self.param_groups):
            ...
            if group['group_type'] >= GROUP_TYPE['multi-head-linear']:
                ...
                group['magnitude_penalize'] = np.intersect1d(topk_indices, group['global_idx']) # find the intersection of two arrays and return the sorted, unique values that are in both of the input arrays
    ...
    def compute_grad_variant(self):
        for i, group in enumerate(self.param_groups):
            ...
            for j, p in enumerate(group['params']):
                if p.grad is None:
                    continue
                refined_grad_f = torch.clone(p.grad.data).detach() # copy grad
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            group['num_steps'] += 1 # cnt steps

        # First pass to compute gradient variant via different criteria
        self.compute_grad_variant() # normal optimizer: sgd / momentum / adam
                ...
                if group['num_groups_g_p'] > 0 and not self.reach_target_group_sparsity():
                    ...
                    if group['num_steps'] >= self.warm_up_steps:
                        ...
                        if not self.reach_target_group_sparsity():
                            ...
                            groups_adjust_lambda = group['magnitude_penalize_bool'].cuda() & (flatten_x_grad_inner_prod < 0) # if cos(θg) < 0, otherwise can be an arbitrary positive constant
